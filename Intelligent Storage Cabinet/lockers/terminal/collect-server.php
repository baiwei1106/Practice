...
if(isset($_GET['box_id']) && isset($_GET['yzm'])){
    $box_id=RemoveXSS($_GET['box_id']);
    $yzm=RemoveXSS($_GET['yzm']);
    ...
    if($flag==2){
            ...
            if($time_flag==1){
                $sql="UPDATE `box` set `flag`=3,`unkonwuserid`=null,`operateflag`=null WHERE `box_id`=?";
                @pdo_query($sql,$box_id) ;
        
                $sql="insert INTO `memory`(`user_id`,`box_id`,`flag`,`date`) VALUES(?,?,?,now())";
                pdo_query($sql,$user,$box_id,0) ;
                require('wait.php');
                require('reload.php');
            }
...
}else if(isset($_GET['yzm'])){
    $yzm=RemoveXSS($_GET['yzm']);


    $sql = "SELECT COUNT(unkonwuserid) AS icount FROM box WHERE unkonwuserid=?";
    $result=pdo_query($sql,$yzm);
    foreach($result as $row){
        $icount=$row['icount'];
    }
    if($icount==1){
        $sql="SELECT * FROM `box` WHERE `unkonwuserid` = ? LIMIT 1";
        $result=pdo_query($sql,$yzm);
        foreach($result as $row){
            $box=$row['box_id'];
        }
        $url="collect-server.php?box_id=".$box."&yzm=".$yzm;
        header("refresh:0;url={$url}");
    }
    else{
        require('identifying_code-error.php');
        exit(0);
    }
}
